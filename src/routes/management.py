...
def manage_teachers():
    ...
    if request.method == 'POST' and action:
        if action == 'delete':
            ...
            teacher_id = request.form.get('teacher_id')
            try:
                        cursor.execute("INSERT INTO teacher (teacher_id, school_id) VALUES (%s, %s)", (teacher_id, session['school_id']))
                        db.commit()
                        flash('Teacher added successfully!', 'success')
            except Exception as e:
                        flash(f'Error adding teacher: {str(e)}', 'error')

    #Rendering Edit Teacher content to Frontend  
...
def manage_subjects():
    ...
    #Rendering Edit Class and Subject content to Frontend
    ...
    return render_template('manage_subjects.html', classes=classes, teachers=teachers, 
                           subjects=subjects, 
                           edit_class_id=edit_class, 
                           edit_subject_id=edit_subject)
...
def manage_timings():
      ...
      if request.method == 'POST':
        ...
        try:
             sql = "Update schools set start_time = %s, end_time = %s, lecture_duration = %s, break_start_time = %s, break_duration = %s where school_id = %s"
        ...
        # Auto-calculate num_lectures and break_after if not provided but config exists
