...
def generate_timetable(class_name, semester, priorities, school_id):
    """Main function to generate timetable based on inputs and priorities."""
    db = connect_db()
    ...
    try:
        ...
        #3 Map Subject -> Credits -> Teacher
        cursor.execute("""
            SELECT s.subject_name, s.credits, t.teacher_id, t.teacher_name
            FROM subject s WHERE s.school_id = %s AND s.class_id = %s AND s.semester = %s
        """, (school_id, class_name, semester))
